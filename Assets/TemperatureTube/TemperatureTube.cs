...
public class TemperatureTube : MonoBehaviour
	...
	private void  Start()
		...
		/** and initialize Capsule Collider for in order for the camera to be able to trace objects
		  * with rays (RayCast), which its center is looking at; this is necessary in order to then
		  * be able to display the parameters of the temperature field
		  */
		CapsuleCollider collider = GetComponent<CapsuleCollider> ();
		
		/** settings for radius and height */
		collider.radius = (float) _outer_radius;
		collider.height = (float) _length;

		/** here we align the collider position with the tube position */
		collider.direction = 2;
	
		/** align the centers of the collider and the tube */
		collider.center = Vector3.forward * (float) _length / 2.0f; 

		/** bind the tube transformation operations to the collider transformation operations */
		...
		canvas = GameObject.Find ("TemperatureCanvas");
